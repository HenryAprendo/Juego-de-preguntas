...
export const verifyOptionSelect = () => {
    const rta = (optionSelected == rtaCorrect) ? true : false;
...
export const add = (data) => {
    ...
    const option_a = document.querySelector('[data-option-a]');
    if (option_a)
        option_a.textContent = data.response.option_a;
    const option_b = document.querySelector('[data-option-b]');
    if (option_b)
        option_b.textContent = data.response.option_b;
    const option_c = document.querySelector('[data-option-c]');
    if (option_c)
        option_c.textContent = data.response.option_c;
    const option_d = document.querySelector('[data-option-d]');
    if (option_d)
        option_d.textContent = data.response.option_d;
};
//# sourceMappingURL=addContent.service.js.map